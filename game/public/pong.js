$(document).ready(() => {
    ...
    let startTime = null;
    //const tiktokChannel = window.env.LIVE_NAME;
    ...
    function anim() {
        ...
        renderBallCombos();
        //renderPlayerLinkToTiktok();
        //console.log(tiktokChannel);
    ...
    function renderEndGame() {
        ...
        const msg = 'Game Over!';
        const msg2 = 'Reload the tab to play again.';
    ...
    /* function renderPlayerLinkToTiktok() {
        const handle = `@${tiktokChannel}`;
        const link = `https://tiktok.com/@${tiktokChannel}`;

        const handleWidth = ctx.measureText(handle).width;
        ctx.fillStyle = 'white';
        ctx.fillText(handle, canvas.width - handleWidth - 10, 30);

        ctx.beginPath();
        ctx.moveTo(canvas.width - handleWidth - 10, 35);
        ctx.lineTo(canvas.width - 10, 35);
        ctx.strokeStyle = 'white';
        ctx.lineWidth = 1;
        ctx.stroke();

        canvas.linkX = canvas.width - handleWidth - 10;
        canvas.linkY = 12;
        canvas.linkWidth = handleWidth;
        canvas.linkHeight = 18;
    }
    ...
    canvas.addEventListener('click', (event) => {
        ...
        if (x >= canvas.linkX && x <= canvas.linkX + canvas.linkWidth &&
            y >= canvas.linkY && y <= canvas.linkY + canvas.linkHeight) {
            window.open(`https://tiktok.com/@${tiktokChannel}`, '_blank');
        }
    }); */
        ...
        if (wall) {
            getBricks(wall.blocks, (block) => {
                if (block.status && block.msg && block.msgLife > 0) {
                    ...
                    ctx.font = "14px Poppins";
                    ctx.fillStyle = 'white';
                    ctx.fillRect(x, y, ctx.measureText(block.msg).width, 25);
